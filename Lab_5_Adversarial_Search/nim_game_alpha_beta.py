...
def minmax_decision(state, alpha, beta):
    ...
    infinity = float('inf')
    state = argmax(successors_of(state), lambda s: min_value(s, alpha, beta))
    return state
...
def main():
    ...
    while not is_terminal(piles):
        piles = minmax_decision(state=piles, alpha=float('inf'), beta=float('-inf'))
